            ...
            <div class="relative p-8 pt-14 mt-4 border border-gray-200 rounded-sm">
                ...
                <form action="{{ route('mahasiswa.update-prestasi', $achievement->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="text" id="competition_name" name="competition_name" required value="{{ old('competition_name', $achievement->competition_name) }}"
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="text" id="competition_name_english" name="competition_name_english" value="{{ old('competition_name_english', $achievement->competition_name_english) }}"
                            class="flex-1 block border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#1e6aae]">
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="text" id="competition_location" name="competition_location" required value="{{ old('competition_location', $achievement->competition_location) }}"
                            class="flex-1 block border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#1e6aae]">
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="text" id="competition_location_english" name="competition_location_english" value="{{ old('competition_location_english', $achievement->competition_location_english) }}"
                            class="flex-1 block border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#1e6aae]">
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="text" id="assignment_letter_number" name="assignment_letter_number" required value="{{ old('assignment_letter_number', $achievement->assignment_letter_number) }}"
                            class="flex-1 block border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#1e6aae]">
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="number" id="pt_partition_number" name="pt_partition_number" required value="{{ old('pt_partition_number', $achievement->pt_partition_number) }}"
                            class="flex-1 block border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#1e6aae]">
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="number" id="partition_number" name="partition_number" required value="{{ old('partition_number', $achievement->partition_number) }}"
                            class="flex-1 block border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#1e6aae]">
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="relative">
                            <select id="place" name="place"
                                ...
                                <option value="1" {{ old('place', $achievement->place) == '1' ? 'selected' : '' }}>Juara 1</option>
                                <option value="2" {{ old('place', $achievement->place) == '2' ? 'selected' : '' }}>Juara 2</option>
                                <option value="3" {{ old('place', $achievement->place) == '3' ? 'selected' : '' }}>Juara 3</option>
                            </select>
                    ...
                    <input type="hidden" id="competition_type" name="competition_type" value="{{ old('competition_type', $achievement->competition_type) }}">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="text" id="competition_url" name="competition_url" value="{{ old('competition_url', $achievement->competition_url) }}"
                            class="flex-1 block border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#1e6aae]">
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <textarea type="text" id="note" name="note"
                            ...
                            placeholder="Tuliskan deskripsi singkat mengenai lomba">{{ old('note', $achievement->note) }}</textarea>
                    </div>
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (existingSupervisorAchievements.length > 0) {
                existingSupervisorAchievements.forEach(dosen => {
                    addDosenRow(dosen.nidn, dosen.role_supervisor_id);
                });
            } else if (oldNidnDosen && oldNidnDosen.length > 0) {
            ...
            function validateFile(fileInput, allowedTypes, maxSize, errorElementId, isRequired = false) {
                ...
                if (fileInput.files.length > 0) {
                    ...
                    const isPdf = allowedTypes.includes('application/pdf');

                    if (isImage && !file.type.startsWith('image/')) {
                        errorElement.textContent = 'File harus berupa gambar.';
                        return false;
                    } else if (isPdf && file.type !== 'application/pdf') {
                    ...
                    if (file.size > maxSize) {
                        errorElement.textContent = `Ukuran file tidak boleh lebih dari ${maxFileSize / (1024 * 1024)}MB.`;
                        return false;
                    }
                } else if (isRequired) {
                    // Only require if there's no existing file and no new file is uploaded
                    const currentFileLink = fileInput.parentNode.querySelector('a');
                    if (!currentFileLink || currentFileLink.getAttribute('href') === '#') {
            ...
            form.addEventListener('submit', function (event) {
                ...
                // For edit page, files are only required if they don't exist or a new one is uploaded
                isValid = validateFile(fileAssignmentLetter, ['application/pdf'], maxFileSize, 'file_assignment_letter_error', true) && isValid;
                isValid = validateFile(fileCertificate, ['application/pdf'], maxFileSize, 'file_certificate_error', true) && isValid;
