...
<main class="flex-1 px-6 pb-10">
    <div class="w-full bg-white mx-auto p-6 border border-gray-200 rounded-lg relative">
        ...
        <div class="flex items-center justify-between mt-6 mb-4">
            ...
            <form method="GET" action="{{ route('dosen.mahasiswa-bimbingan') }}" class="flex items-center space-x-2 mb-4">
                <input
                    type="text"
                    name="search"
                    value="{{ request('search') }}"
                    placeholder="Cari lomba atau mahasiswa..."
                    class="border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-[#1e6aae]">
                <button
                    type="submit"
                    class="px-3 py-1 rounded bg-[#1e6aae] text-white text-sm font-medium hover:bg-[#155a9c] transition-colors">
                    Cari
                </button>
                @if(request('search'))
                <a href="{{ route('dosen.mahasiswa-bimbingan') }}" class="text-xs text-gray-500 hover:underline ml-2">Reset</a>
                @endif
            </form>
        </div>
