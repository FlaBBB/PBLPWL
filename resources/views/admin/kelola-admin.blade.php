...
@section('content')
    <main class="flex-1 px-10">
        <div class="w-full mx-auto p-6  border border-gray-200 rounded-lg">
            <h2 class="text-xl font-semibold">Kelola Admin</h2>
                ...
                <table class="w-full text-left text-sm bg-white rounded-lg border border-gray-200 rounded-sm">
                    <thead class="text-gray-500">
                        <tr class="border-b border-gray-200">
                            ...
                            <th class="w-[15%] px-2 py-2 text-left">NIP</th>
                            <th class="w-[20%] px-2 py-2 text-left">Nama Admin</th>
                    ...
                    <tbody>
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-50">
                                ...
                                <td class="px-2 py-2 flex justify-center gap-2">
                                    <div class="flex space-x-2">
                                        ...
                                        <button type="button" onclick="openDeleteModal('{{ $adm->nip }}', '{{ $adm->name }}', '{{ $adm->user->email ?? '-' }}')"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-2 text-center text-gray-500">Tidak ada data admin.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div id="modal-detail"
            ...
            <div
                ...
                <h3 class="text-xl font-semibold mb-6 text-gray-800">Detail Admin</h3>

                <div class="space-y-4">
                    <table class="w-full text-gray-800 text-left text-sm">
                        <tr>
                            <td class="border border-gray-300 px-3 py-2 font-medium w-1/3 bg-gray-50">NIP</td>
                            <td class="border border-gray-200 px-3 py-2" id="detail-nip"></td>
                        </tr>
                        <tr>
        ...
        <div id="modal-hapus"
            ...
            <div
                ...
                <div class="text-center">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-2">Hapus Data Admin</h3>
                    <div class="mb-4 space-y-4">
                        <p class="text-sm text-gray-500 mb-2">
                            Apakah Anda yakin ingin menghapus data admin berikut?
                        </p>
                        <div class="bg-gray-50 p-3 space-y-2 rounded-lg text-left">
                            <div class="text-sm"><strong>Nama:</strong> <span id="delete-admin-nama"></span></div>
                            <div class="text-sm"><strong>NIP:</strong> <span id="delete-admin-nip"></span></div>
                            <div class="text-sm"><strong>Email:</strong> <span id="delete-admin-email"></span></div>
                        </div>
                    </div>
        ...
        <div id="modal-edit"
            ...
            <div
                ...
                <h3 class="text-xl font-semibold mb-6 text-gray-800">Edit Data Admin</h3>
                        ...
                        <table class="w-full text-gray-800 text-left text-sm">
                            <tr>
                                ...
                                <td class="border border-gray-200 px-3 py-2">
                                    <input type="text" id="edit-nip" name="nip" value="{{ old('nip') }}"
        ...
        <div id="modal-tambah"
            ...
            <h3 class="text-xl font-semibold mb-6 text-gray-800">Tambah Data Admin</h3>
 
            <form id="addForm" method="POST" action="{{ route('admin.kelola-admin.store') }}">
                @csrf
                <div class="space-y-4">
                <table class="w-full text-gray-800 text-left text-sm">
                    <tr>
                    <td class="border border-gray-300 px-3 py-2 font-medium w-1/3 bg-gray-50">NIP</td>
                    <td class="border border-gray-200 px-3 py-2">
                        <input type="text" id="add-nip" name="nip" value="{{ old('nip') }}"
                        class="w-full px-2 py-1 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#1e6aae] focus:border-transparent text-sm">
                    </td>
                    </tr>
                    <tr>
    ...
    <script>
        ...
        function openEditModalFromButton(button) {
            document.getElementById('edit-nip').value = button.dataset.id;
            document.getElementById('edit-nama').value = button.dataset.name;
            document.getElementById('edit-email').value = button.dataset.email;
 
 
            document.getElementById('editForm').action = `{{ url('admin/kelola-pengguna/admin') }}/${button.dataset.id}`;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // For Edit Modal - Removed preferences logic
            // For Add Modal - Removed preferences logic
 
            // Close the dropdown if the user clicks outside of it - Removed preferences dropdowns
            window.onclick = function(event) {
                // No dropdowns to handle for admin
            }
 
        });
 
        // Update openEditModalFromButton to handle the new preference display - Removed preferences logic
        ...
        openEditModalFromButton = function(button) {
            ...
            // If there are old values from a failed edit submission, pre-populate them
            document.getElementById('edit-nip').value = "{{ old('nip') }}" || button.dataset.id;
        ...
        // Update openModal for add modal to reset preferences display - Removed preferences logic
    ...
    @if ($errors->any() && (old('nip') || old('name') || old('email')))
        <script>
            document.addEventListener('DOMContentLoaded', function() {
    ...
    @if ($errors->any() && old('edit_id'))
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const editButton = document.querySelector(`button[data-id="${editId}"]`);
                if (editButton) {
                    openEditModalFromButton(editButton);
                } else {
                    console.log('Edit button not found for ID:', editId);
                }
            });
        </script>
    @endif
